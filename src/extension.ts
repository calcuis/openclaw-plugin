    ...
    const menuCommand = vscode.commands.registerCommand('claw.showMenu', async () => {
        ...
        if (selection) {
            ...
            if (selection === 'Setup') {
                ...
                } else if (setupSelection === 'Fix') {
                    await runClawCommand(context, 'openclaw doctor --fix');
        ...
        try {
            ...
            if (!installedVersion) {
                ...
                if (selection === 'Install Openclaw') {
                    ...
                    await runClawCommand(context, 'curl -fsSL https://openclaw.ai/install.sh | bash');
                }
                return;
            }
...
async function launchDashboard(context: vscode.ExtensionContext) {
    const isWindows = os.platform() === 'win32';

    await vscode.window.withProgress({
        location: vscode.ProgressLocation.Notification,
        title: 'Starting Open Claw Control...',
        cancellable: false
    }, () => new Promise<void>((resolve) => {
        const cmd = isWindows ? 'wsl.exe' : 'openclaw';
        const args = isWindows ? ['-d', 'Ubuntu', 'openclaw', 'dashboard'] : ['dashboard'];
        ...
        const tryOpen = (text: string) => {
            const match = text.match(/https?:\/\/127\.0\.0\.1:\d+[^\s\n]*/);
            if (match) {
                openBrowser(match[0]);
            }
        };

        child.stdout?.on('data', (data: Buffer) => { outputBuffer += data.toString(); tryOpen(outputBuffer); });
        child.stderr?.on('data', (data: Buffer) => { outputBuffer += data.toString(); tryOpen(outputBuffer); });

        child.on('close', () => {
            if (!resolved) {
                // Dashboard likely started as a background daemon — wait briefly for the server to start
                setTimeout(() => openBrowser('http://127.0.0.1:18789/'), 2000);
            }
        });

        // Fallback: open base URL after 10 seconds if no URL ever appears in output
        setTimeout(() => openBrowser('http://127.0.0.1:18789/'), 10000);
    }));
}
